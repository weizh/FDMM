...
public class HMM_MLModel extends MLModel {
	...
	public static HMM_MLModel load(String file) throws IOException, ClassNotFoundException {
		FileInputStream in = new FileInputStream(file);
		ObjectInputStream ins = new ObjectInputStream(in);
		HMM_MLModel copy = (HMM_MLModel) ins.readObject();
		in.close();
		ins.close();
		return copy;
	}
	...
	public void add(Sentence s, boolean lemmatized) {
		...
		for (int i = 0; i <= s.getWords().size(); i++) {
			if (i > 0 && i < s.getWords().size()) {
				...
				prevpos = s.getWords().get(i - 1).getPOS();
				curw = s.getWords().get(i).getLemma();
				curpos = s.getWords().get(i).getPOS();
				if (lemmatized)
