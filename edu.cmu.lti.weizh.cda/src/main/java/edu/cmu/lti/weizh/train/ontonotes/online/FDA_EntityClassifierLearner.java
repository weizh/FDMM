...
import edu.cmu.lti.weizh.train.ontonotes.cli.FDA_Inferencer;

public class FDA_EntityClassifierLearner {

	FDMM posModel;
	static String modelname, path;

	public FDA_EntityClassifierLearner(FDMM posModel2) {
		posModel = posModel2;
	}
	...
	public static void main(String argv[]) throws Exception {
		...
		FDA_EntityClassifierLearner onlineLearner = new FDA_EntityClassifierLearner(FDMM.load("POS-rich-randomSent-fold10.0-NOV08.en.FDA_MLModel"));
		...
		while ((line = br.readLine()) != null) {
			...
			while ((line = br.readLine()) != null) {
				if (line.equalsIgnoreCase("D"))
					...
					if (ONF_CONSTS.EntityList.contains(toks[1]) == false) {
						System.err.println("Entity Type not in List. Entity should be in the list below:");
						System.err.println(ONF_CONSTS.EntityList);
						continue;
					}
			...
			for (Word word : words) {
				...
				for (String s : new String[] { Stemmer.stemTerm(ctok), cform, cpos, ccap, cform + "_" + cpos }) {
					...
					fdamodel.add(s, word.getEntityType(), ONF_CONSTS.CTOK, sctok);
