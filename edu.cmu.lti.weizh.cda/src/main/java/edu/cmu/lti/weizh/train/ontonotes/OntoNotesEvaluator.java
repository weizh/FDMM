...
public class OntoNotesEvaluator implements Evaluatable<FDMM, OntonotesDataSet> {
	...
	public static void main(String arv[]) throws Exception {
		...
		if (type.equals(EVAL_CONSTS.NER_TYPE)) {
			new OntoNotesEvaluator().infer(mode, TbT, FDMM.load("NER10-rich-randomSent-fold10.0-NOV08.en.FDA_MLModel"),
					dataset, foldId);
			...
			new OntoNotesEvaluator().infer(mode, TbT, model, dataset, foldId);
			dataset.printTokenByTokenEvaluation(foldId);
		}
