...
public class ONF_DataSet extends DataSet {
	...
	public void predictionsNE_EntitybyEntity_Stat() throws Exception {
		...
		System.out.printf("Average f1 of all class is: %.4f", accf1
				/ (double) n);
	}

	public void predictions_NE_TokenbyToken_Stat(int foldId) throws Exception {
				...
				for (Sentence sent : para.getSentences()) {
					...
					for (Word word : words) {
						if (word.getBooleanFolds()[foldId])
							allTrainStat.adjustOrPutValue(word.getEntityType(),
									1, 1);
						else
							allTestStat.adjustOrPutValue(word.getEntityType(),
									1, 1);
						if (word.getBooleanFolds()[foldId])
							continue;
	...
	public void predictions_POS_TokenbyToken_Stat(int foldId) throws Exception {
				...
				for (Sentence sent : para.getSentences()) {
					...
					for (Word word : words) {
						...
						if (word.getBooleanFolds()[foldId])
							continue;
						if (word.getPrediction() == null){
							System.err.println("Not predicted word "+word.getLemma());
//							System.err.println();
						}
